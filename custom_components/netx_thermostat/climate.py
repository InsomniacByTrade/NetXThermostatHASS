...
class NetXClimate(CoordinatorEntity, ClimateEntity):
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode):
        ...
        if hvac_mode == HVACMode.HEAT:
            ...
            await self._set_fan("AUTO")  # Turn off manual fan when switching to heat
        elif hvac_mode == HVACMode.COOL:
            await self._set_mode("COOL")
            await self._set_fan("AUTO")  # Turn off manual fan when switching to cool
        elif hvac_mode == HVACMode.FAN_ONLY:
            await self._set_fan_only()
        elif hvac_mode == HVACMode.OFF:
            await self._set_mode("OFF")
            await self._set_fan("AUTO")  # Turn off manual fan when turning off
    ...
    async def _set_fan(self, fan_mode: str):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, data=data, auth=auth, timeout=aiohttp.ClientTimeout(total=10)) as response:
                if response.status != 200:
                    _LOGGER.error("Failed to set fan mode: %s", response.status)
